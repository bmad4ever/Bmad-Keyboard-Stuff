...
#    endif
    ...
    switch (action.kind.id) {
        ...
        case ACT_RMODS_TAP: {
            ...
            uint8_t active_osm = get_oneshot_mods();
            uint8_t active_locked = get_oneshot_locked_mods();
                ...
                case MODS_ONESHOT:
                    ...
                    if (event.pressed) {
                        if (tap_count == 0) {
                            ...
                            if(mods & active_locked){
                                clear_oneshot_mods();
                                clear_oneshot_locked_mods();
                                set_oneshot_locked_mods((~mods) & active_locked);
                                unregister_mods(mods);
                                set_oneshot_mods((~mods) & active_osm);
                            }
                            else if(mods & active_osm)
                            {
                                clear_oneshot_mods();
                                set_oneshot_locked_mods( mods | active_locked);
                                register_mods(mods);
